...
class AuthWindow(QDialog):
    ...
    def _fetch_user_data(self, email: str | None = None):
        """Pobiera dane użytkownika po weryfikacji"""
        # Jeśli mamy tokeny, możemy spróbować pobrać dane z API
        # Na razie używamy podstawowych danych
        if not email:
            email = self.register_email.text() or self.login_email.text()
        
        self.user_data = {
            "name": "Użytkownik",  # TODO: Pobierz z API używając access_token
            "email": email,
            "language": "pl",
            "timezone": "Europe/Warsaw"
        }
