...
class BaseHabbitDialog(QDialog):
    ...
    def setup_base_ui(self):
        ...
        # Nagłówek
        title_label = QLabel(f"Edytuj nawyk: {self.habit_name}")
        ...
        date_layout.addRow("Wybierz datę:", self.date_edit)
...
class CheckboxHabbitDialog(BaseHabbitDialog):
    ...
    def __init__(self, parent=None, habit_name="", current_value="", max_date=None):
        ...
        self.setWindowTitle("Odznacz nawyk")
        self.setup_content()
        
    def setup_content(self):
        """Tworzy kontrolki dla typu checkbox"""
        info_group = QGroupBox("Akcja")
        info_layout = QVBoxLayout(info_group)
        
        info_label = QLabel("Naciśnięcie OK oznacza wykonanie tego nawyku w wybranym dniu.")
        info_label.setWordWrap(True)
        info_layout.addWidget(info_label)
        
        # Status aktualny
        current_status = "Wykonano" if self.current_value == "1" else "Nie wykonano"
        status_label = QLabel(f"Aktualny status: {current_status}")
...
class CounterHabbitDialog(BaseHabbitDialog):
    ...
    def setup_content(self):
        ...
        counter_layout.addRow("Dodaj/odejmij:", self.value_spinbox)
...
class DurationHabbitDialog(BaseHabbitDialog):
    ...
    def __init__(self, parent=None, habit_name="", current_value="", max_date=None):
        super().__init__(parent, habit_name, current_value, max_date)
        self.setWindowTitle("Czas trwania")
    ...
    def setup_content(self):
        ...
        # Ręczne wprowadzanie
        manual_group = QGroupBox("Lub wprowadź ręcznie")
        ...
        manual_layout.addRow("Czas:", self.time_edit)
...
class TimeHabbitDialog(BaseHabbitDialog):
    ...
    def __init__(self, parent=None, habit_name="", current_value="", max_date=None):
        super().__init__(parent, habit_name, current_value, max_date)
        self.setWindowTitle("Godzina")
        self.setup_content()
        
    def setup_content(self):
        """Tworzy kontrolki dla typu godzina"""
        time_group = QGroupBox("Godzina")
        time_layout = QFormLayout(time_group)
        
        # Aktualna wartość
        if self.current_value:
            current_label = QLabel(f"Aktualna godzina: {self.current_value}")
            current_label.setStyleSheet("font-weight: bold;")
        ...
        time_layout.addRow("Nowa godzina:", self.time_edit)
...
class ScaleHabbitDialog(BaseHabbitDialog):
    ...
    def __init__(self, parent=None, habit_name="", current_value="", scale_max=10, max_date=None):
        ...
        self.setWindowTitle("Skala")
        self.setup_content()
        
    def setup_content(self):
        """Tworzy kontrolki dla typu skala"""
        scale_group = QGroupBox("Skala")
        scale_layout = QFormLayout(scale_group)
        
        # Aktualna wartość
        if self.current_value:
            current_label = QLabel(f"Aktualna wartość: {self.current_value}")
            current_label.setStyleSheet("font-weight: bold;")
        ...
        scale_layout.addRow("Wartość:", self.value_label)
        scale_layout.addRow("Wybierz:", self.slider)
...
class TextHabbitDialog(BaseHabbitDialog):
    ...
    def update_char_count(self):
        ...
        self.char_count_label.setText(f"Liczba znaków: {count}")
...
class AddHabbitDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        # Formularz
        form_group = QGroupBox("Nowy nawyk")
        form_layout = QFormLayout(form_group)
        
        # Nazwa nawyku
        self.name_edit = QLineEdit()
        self.name_edit.setMinimumHeight(35)
        self.name_edit.setPlaceholderText("Np. Ćwiczenia, Czytanie, Medytacja...")
        form_layout.addRow("Nazwa nawyku:", self.name_edit)
        ...
        form_layout.addRow("Typ nawyku:", self.type_combo)
        ...
        form_layout.addRow("Maksymalna skala:", self.scale_spinbox)
        ...
        # Opis typów
        info_group = QGroupBox("Opis typów")
        info_layout = QVBoxLayout(info_group)
        
        info_text = """
• odznacz - Checkbox do zaznaczania wykonania
• Ile razy - Licznik zwiększany przy każdym wykonaniu  
• czas trwania - Śledzenie czasu spędzonego na nawyku
• Godzina - Zapisywanie konkretnej godziny wykonania
• Skala - Ocena jakości/intensywności (np. 7/10)
• tekst - Notatki tekstowe związane z nawykiem
        """
...
class RemoveHabbitDialog(QDialog):
    ...
    def __init__(self, parent=None, habits_list=None):
        ...
        self.setWindowTitle("Usuń nawyk")
        self.setModal(True)
        self.setMinimumWidth(300)
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        if not self.habits_list:
            label = QLabel("Brak nawyków do usunięcia.")
        ...
        # Wybór nawyku
        habit_group = QGroupBox("Wybierz nawyk do usunięcia")
        ...
        habit_layout.addRow("Nawyk:", self.habit_combo)
        layout.addWidget(habit_group)
        
        # Ostrzeżenie
        warning_group = QGroupBox("Uwaga")
        warning_layout = QVBoxLayout(warning_group)
        
        warning_text = QLabel("⚠️ Usunięcie nawyku spowoduje trwałe usunięcie wszystkich powiązanych danych!")
