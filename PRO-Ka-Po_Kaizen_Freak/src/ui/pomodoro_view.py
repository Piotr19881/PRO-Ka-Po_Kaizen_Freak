    ...
    def _apply_theme(self):
        ...
        self.setStyleSheet(f"""
            ...
            QCheckBox::indicator {{
                width: 18px;
                height: 18px;
                border: 2px solid {colors.get('checkbox_border', '#3498db')};
                border-radius: 3px;
                background-color: {bg_main};
            }}

            QCheckBox::indicator:checked {{
                background-color: {colors.get('checkbox_checked', '#27ae60')};
                border-color: {colors.get('checkbox_checked', '#27ae60')};
            }}

            QCheckBox::indicator:hover {{
                border-color: {colors.get('checkbox_checked_hover', '#229954')};
            }}
        """)

        logger.info("[PomodoroView] Theme applied successfully")
