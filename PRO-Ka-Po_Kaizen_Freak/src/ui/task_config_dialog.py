...
class TaskConfigDialog(QDialog):
    ...
    def __init__(self, parent: Optional[QWidget] = None, local_db=None, sync_manager=None):
        super().__init__(parent)
        
        # Baza danych lokalna
        self.local_db = local_db
        # Optional sync manager (TasksSyncManager) to enqueue custom list changes
        ...
        if self.local_db:
            try:
                ...
                if self.local_db.save_custom_lists(config['custom_lists']):
                    ...
                    # Enqueue custom lists for background sync so changes propagate to server/devices
                    try:
                        if self.sync_manager:
                            db_lists = self.local_db.load_custom_lists()
                            for lst in db_lists:
                                local_id = lst.get('id')
                                server_uuid = lst.get('server_uuid') or str(uuid.uuid4())
                                # Queue using existing server_uuid or generated uuid
                                try:
                                    self.sync_manager.queue_custom_list(server_uuid, local_id, action='upsert')
                                except Exception as e:
                                    logger.warning(f"Failed to queue custom list {local_id} for sync: {e}")
                    except Exception as e:
                        logger.debug(f"Could not enqueue custom lists for sync: {e}")
                else:
