...
class EmailConnector:
    ...
    def _connect_imap(self) -> bool:
        ...
        try:
            ...
            # Wybierz klasę IMAP
            if use_ssl:
                # pass a socket timeout to avoid blocking indefinitely
                try:
                    self.connection = imaplib.IMAP4_SSL(server, port, timeout=timeout)
                except TypeError:
                    # Older Python versions may not accept 'timeout' keyword; fall back
                    self.connection = imaplib.IMAP4_SSL(server, port)
                logger.debug(f"[EmailConnector] IMAP SSL connection to {server}:{port} (timeout={timeout}s)")
            else:
                try:
                    self.connection = imaplib.IMAP4(server, port, timeout=timeout)
                except TypeError:
                    self.connection = imaplib.IMAP4(server, port)
                logger.debug(f"[EmailConnector] IMAP connection to {server}:{port} (timeout={timeout}s)")
