...
import sqlite3
try:
    # Use packaging-aware data dir from config when available
    from src.core.config import config
except Exception:
    # Fallback: resolve relative path at runtime
    config = None
import json
...
class AssistantDatabase:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        """
        # Prefer explicit db_path, otherwise use config.DATA_DIR (packaging-aware)
        if db_path:
            self.db_path = Path(db_path)
        else:
            if config is not None:
                self.db_path = Path(config.DATA_DIR) / "assistant.db"
            else:
                self.db_path = Path("data") / "assistant.db"
