...
# Import global config (API base URL defaults)
try:
    from ....config import API_BASE_URL
except Exception:
    # Fallback if package imports differ
    try:
...
class ShareFileDialog(QDialog):
    ...
    def __init__(self, file_name, parent=None):
        ...
        # Use global API_BASE_URL from config as default (falls back to localhost)
        try:
            self.api_input.setText(API_BASE_URL)
        except Exception:
            self.api_input.setText("http://localhost:8000")
        self.api_input.setPlaceholderText(f"{API_BASE_URL} lub http://localhost:8000")
