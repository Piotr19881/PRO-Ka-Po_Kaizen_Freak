...
class AlarmManager:
    ...
    def __init__(
        ...
        self.on_alarm_deleted: Optional[Callable[[str], None]] = None
        self.on_timer_changed: Optional[Callable[[Timer], None]] = None
        self.on_timer_deleted: Optional[Callable[[str], None]] = None
    ...
    def _init_sync(self, api_base_url: str, auth_token: str):
        ...
        try:
            ...
            self.sync_manager = SyncManager(
                ...
                sync_interval=300  # Sync co 5 minut (zmienione z 30 sekund dla optymalizacji)
            )
    ...
    def set_ui_callbacks(
        ...
        on_alarm_deleted: Optional[Callable[[str], None]] = None,
        on_timer_changed: Optional[Callable[[Timer], None]] = None,
        on_timer_deleted: Optional[Callable[[str], None]] = None
    ):
        """
        Ustaw callbacki dla UI updates.

        Args:
            on_alarm_changed: Callback wywoływany gdy alarm się zmieni (create/update)
            on_alarm_deleted: Callback wywoływany gdy alarm zostanie usunięty
            on_timer_changed: Callback wywoływany gdy timer się zmieni
            on_timer_deleted: Callback wywoływany gdy timer zostanie usunięty
        ...
        Example:
            ...
            def remove_alarm_from_ui(alarm_id: str):
                # Usuń alarm z UI
                self.alarm_view.remove_alarm(alarm_id)

            alarm_manager.set_ui_callbacks(
                on_alarm_changed=refresh_alarm_list,
                on_alarm_deleted=remove_alarm_from_ui
            )
        """
        self.on_alarm_changed = on_alarm_changed
        self.on_alarm_deleted = on_alarm_deleted
        self.on_timer_changed = on_timer_changed
        self.on_timer_deleted = on_timer_deleted
