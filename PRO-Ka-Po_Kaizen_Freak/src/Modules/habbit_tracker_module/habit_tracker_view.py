...
class HabbitTrackerView(QWidget):
    ...
    def __init__(self, db_manager=None, parent=None):
        ...
        if SYNC_AVAILABLE and self.db_manager:
            try:
                ...
                self.sync_manager = HabitSyncManager(
                    habit_db=self.db_manager,  # db_manager już jest HabitDatabase
                    api_client=api_client,
                    sync_interval=300  # Automatyczna synchronizacja co 5 minut (zmienione z 30 sekund dla optymalizacji)
                )

                # Nie uruchamiaj jeszcze synchronizacji - zostanie uruchomiona w set_user_data po ustawieniu user_id
                logger.info("[HABIT] ✅ Sync manager utworzony - synchronizacja zostanie uruchomiona po ustawieniu user_id")
        ...
        if self.theme_manager:
            ...
            self.colors = {}
            logger.warning("[HABIT] Theme manager not available, using default colors")
        ...
        # Odśwież tabelę ponownie po aplikacji motywu, aby kolory weekendów zostały zastosowane
        self.refresh_table()
    ...
    def create_habits_table(self, parent_layout):
        ...
        # Wyłącz automatyczne kolorowanie wierszy - będziemy robić to ręcznie dla weekendów
        self.habits_table.setAlternatingRowColors(False)
    ...
    def refresh_table(self):
        ...
        for day in range(1, days_in_month + 1):
            ...
            # HARDCODE kolory weekendów dla spójności (niezależne od motywu) - ulepszone podświetlenie
            saturday_color = QColor("#90EE90")  # Jasnozielony dla soboty (lepszy kontrast)
            sunday_color = QColor("#FFB6C1")    # Jasnoróżowy dla niedzieli (lepszy kontrast)
            weekend_text_color = QColor("#2F4F4F")  # Ciemnoszary tekst dla lepszej czytelności
            ...
            for col, habit in enumerate(self.habits, 2):
                ...
                # Wszystkie typy nawyków traktujemy jako QTableWidgetItem dla spójnego kolorowania wierszy
                ...
                # Kolorowanie weekendów dla całego wiersza - HARDCODE dla spójności
                if weekday == 5:  # Sobota
                    item.setBackground(QBrush(QColor("#90EE90")))  # Jasnozielony (lepszy kontrast)
                    item.setForeground(QBrush(QColor("#2F4F4F")))  # Ciemnoszary tekst
                elif weekday == 6:  # Niedziela
                    item.setBackground(QBrush(QColor("#FFB6C1")))  # Jasnoróżowy (lepszy kontrast)
                    item.setForeground(QBrush(QColor("#2F4F4F")))  # Ciemnoszary tekst
    ...
    def apply_theme(self):
        ...
        disabled_bg = self.colors.get("disabled_bg", "#cccccc")
        disabled_text = self.colors.get("disabled_text", "#666666")
