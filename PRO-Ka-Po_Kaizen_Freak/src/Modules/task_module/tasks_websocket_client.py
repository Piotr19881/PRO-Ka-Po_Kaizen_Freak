...
class TasksWebSocketClient(QThread):
    ...
    def update_token(self, new_token: str):
        ...
        logger.info(f"🔄 Tasks WebSocket token updated (changed: {old_token != new_token})")
        
        # Jeśli WebSocket jest aktywnie połączony, zrestartuj połączenie z nowym tokenem
        if self._websocket and not self._websocket.closed:
            logger.info("🔌 Restarting Tasks WebSocket connection with new token...")
                ...
                async def close_and_reconnect():
                    try:
                        if ws_to_close and not ws_to_close.closed:
                            ...
                            logger.info("✅ Old Tasks WebSocket connection closed, will reconnect with new token")
                    except Exception as e:
                        logger.warning(f"Error closing old Tasks connection: {e}")
                
                asyncio.run_coroutine_threadsafe(close_and_reconnect(), self._loop)
        else:
            logger.debug("Tasks WebSocket not connected, token will be used on next connection attempt")
