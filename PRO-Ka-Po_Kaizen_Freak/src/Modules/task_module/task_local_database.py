...
class TaskLocalDatabase:
    ...
    def mark_custom_list_synced(self, list_id: int, server_uuid: str, version: Optional[int] = None) -> bool:
        """
        Oznacz lokalną listę własną jako zsynchronizowaną z serwerem.

        Args:
            list_id: Lokalny ID listy w task_custom_lists
            server_uuid: UUID nadany przez serwer (lub użyty przy wysyłce)
            version: Opcjonalna wersja synchronizacji
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                updates = ["server_uuid = ?", "synced_at = CURRENT_TIMESTAMP"]
                params = [server_uuid]
                ...
                """, params)
                conn.commit()
                logger.info(f"[TASK DB] Marked custom list {list_id} as synced (server_uuid={server_uuid})")
                return True

        except Exception as e:
            logger.error(f"[TASK DB] Failed to mark custom list synced: {e}")
            return False

    def upsert_custom_list_from_server(self, server_uuid: str, name: str, values: List[str], version: int = 1, created_at: Optional[str] = None, updated_at: Optional[str] = None) -> bool:
        """
        Insert or update a custom list coming from the server. Uses server_uuid to match existing rows.

        Args:
            server_uuid: Server-side UUID of the list
            name: Name of the list
            values: List of string values
            version: Version number from server
            created_at: Optional created timestamp
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                # Check if a row with this server_uuid exists
                cursor.execute("SELECT id FROM task_custom_lists WHERE server_uuid = ? AND user_id = ?", (server_uuid, self.user_id))
                row = cursor.fetchone()
                ...
                if row:
                    ...
                    cursor.execute("""
                        UPDATE task_custom_lists
                        SET name = ?, list_values = ?, version = ?, synced_at = CURRENT_TIMESTAMP, updated_at = ?
                        WHERE id = ? AND user_id = ?
                    """, (name, values_json, int(version or 1), updated_at or now, local_id, self.user_id))
                else:
                    cursor.execute("""
                        INSERT INTO task_custom_lists (user_id, name, list_values, server_uuid, version, synced_at, created_at, updated_at)
                        VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP, ?, ?)
                    """, (self.user_id, name, values_json, server_uuid, int(version or 1), created_at or now, updated_at or now))

                conn.commit()
                logger.info(f"[TASK DB] Upserted custom list from server: {name} (server_uuid={server_uuid})")
                return True

        except Exception as e:
            logger.error(f"[TASK DB] Failed to upsert custom list from server: {e}")
            return False
    ...
    def get_tasks(self, parent_id: int = None, include_archived: bool = False,
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # If caller requested only top-level tasks, filter parent_id IS NULL.
                # If caller wants subtasks included (include_subtasks=True) and
                # didn't pass an explicit parent_id, do not filter by parent_id so
                # we can return both parents and subtasks. If parent_id is set,
                # filter for that parent only.
                if parent_id is None:
                    if not include_subtasks:
                        query += " AND parent_id IS NULL"
                    # else: include_subtasks==True -> do not filter, return all tasks
                else:
                    query += " AND parent_id = ?"
