...
class TasksAPIClient:
    ...
    def bulk_sync(
        ...
        custom_lists: List[Dict[str, Any]] = None
    ) -> APIResponse:
        """
        ...
        try:
            ...
            logger.debug(f"Bulk sync: {len(payload['tasks'])} tasks, {len(payload['tags'])} tags, {len(payload['kanban_items'])} kanban items, {len(payload['custom_lists'])} custom lists")
    ...
    def list_custom_lists(self, user_id: str, include_deleted: bool = False) -> APIResponse:
        """Fetch custom lists from server"""
        try:
            params = {'user_id': user_id, 'include_deleted': include_deleted}
            # Try common endpoint names, be tolerant to server shapes
            endpoints = [
                f"{self.base_url}/api/tasks/custom-lists",
                f"{self.base_url}/api/tasks/custom-list",
                f"{self.base_url}/api/tasks/custom-lists/list",
            ]
            last_error = None
            for url in endpoints:
                try:
                    response = self._request_with_retry('GET', url, params=params)
                    # Treat any 2xx/200 as success
            ...
            # If none succeeded, return last error wrapped
            if isinstance(last_error, requests.Response):
                return self._handle_response(last_error)
            return APIResponse(success=False, error=str(last_error))
        except Exception as e:
            logger.error(f"Error fetching custom lists: {e}")
            return APIResponse(success=False, error=str(e))

    def sync_custom_list(self, list_data: Dict[str, Any], user_id: str) -> APIResponse:
        """Sync a single custom list using per-item endpoints as fallback.

        Tries a small set of common endpoints; returns the first successful response.
        """
        try:
            payload = {**list_data, 'user_id': user_id}
            # Prepare candidate endpoints (most specific first)
            endpoints = [
                f"{self.base_url}/api/tasks/custom-list",
                f"{self.base_url}/api/tasks/custom-lists",
                f"{self.base_url}/api/tasks/custom-lists/item",
            ]
            last_error = None
            for url in endpoints:
                try:
            ...
            logger.error(f"Error syncing custom list: {e}")
            return APIResponse(success=False, error=str(e))
