...
class BackblazeService:
    ...
    def __init__(self):
        # Sprawdzenie czy b2sdk jest zainstalowane
        if InMemoryAccountInfo is None or B2Api is None or UploadSourceBytes is None:
            logger.error("b2sdk is not installed. Please install it with: pip install b2sdk")
            raise RuntimeError("b2sdk module is required but not installed")
    ...
    def _authorize(self):
        ...
        try:
            ...
            # Jeśli download_url nie jest ustawiony, pobierz z bucket
            if not self.download_url or self.download_url.strip() == "":
                # Pobierz publiczny URL bucketa
                # Format: https://f<bucket_id>.backblazeb2.com/file/<bucket_name>
                account_info = self.b2_api.account_info
                download_url_from_api = account_info.get_download_url()
                
                # Konstruuj pełny URL
                self.download_url = f"{download_url_from_api}/file/{self.bucket_name}"
                logger.info(f"Auto-detected download URL: {self.download_url}")
